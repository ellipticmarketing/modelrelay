...
export async function runOnboard(port = 7352) {
  ...
  for (const providerKey of PROVIDER_ORDER) {
    ...
    if (providerKey === 'kilocode') {
      const authEnabled = isKiloCodeBearerEnabled(config)
      console.log(`  bearer auth: ${authEnabled ? 'enabled' : 'disabled'} (token is optional)`)
    }
    ...
    if (value === '-') {
      ...
      if (providerKey === 'kilocode') {
        const authAnswer = await rl.question(`  attach bearer token to KiloCode requests? [${isKiloCodeBearerEnabled(config) ? 'Y/n' : 'y/N'}]: `)
        config.providers[providerKey].useBearerAuth = parseYesNo(authAnswer, isKiloCodeBearerEnabled(config))
      }
