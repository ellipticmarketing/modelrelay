...
export function parseArgs(argv) {
  ...
  const hasConfigFlag = args.some(a => a.toLowerCase() === '--config')
  const hasDryRunFlag = args.some(a => a.toLowerCase() === '--dry-run')
  const hasYesFlag = args.some(a => a.toLowerCase() === '--yes')
  const hasYesShortFlag = args.some(a => a.toLowerCase() === '-y')
  ...
  const configIdx = args.findIndex(a => a.toLowerCase() === '--config')
  const configValueIdx = (configIdx !== -1 && args[configIdx + 1] && !args[configIdx + 1].startsWith('--'))
    ? configIdx + 1
    : -1
  ...
  let configPath = null
  if (configValueIdx !== -1) {
    configPath = args[configValueIdx]
  }

  const dryRun = hasDryRunFlag || hasConfigFlag
  const yes = hasYesFlag || hasYesShortFlag
