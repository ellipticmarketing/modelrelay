...
export const getP95 = (r, windowMs = DEFAULT_PING_WINDOW_MS) => {
  const now = Date.now()
  const successfulPings = (r.pings || [])
    .filter(p => p.code === '200' && (p.ts == null || now - p.ts <= windowMs))
    .map(p => p.ms)
  ...
  const index = Math.ceil(successfulPings.length * 0.95) - 1
  return successfulPings[Math.max(0, index)]
}

export const getJitter = (r, windowMs = DEFAULT_PING_WINDOW_MS) => {
  const now = Date.now()
  const successfulPings = (r.pings || [])
    .filter(p => p.code === '200' && (p.ts == null || now - p.ts <= windowMs))
    .map(p => p.ms)
  if (successfulPings.length < 2) return null
  const mean = successfulPings.reduce((a, b) => a + b, 0) / successfulPings.length
  const variance = successfulPings.reduce((sum, ms) => sum + Math.pow(ms - mean, 2), 0) / successfulPings.length
  return Math.round(Math.sqrt(variance))
}

export const getSpikeRate = (r, windowMs = DEFAULT_PING_WINDOW_MS) => {
  const now = Date.now()
  const successfulPings = (r.pings || [])
    .filter(p => p.code === '200' && (p.ts == null || now - p.ts <= windowMs))
    .map(p => p.ms)
    .sort((a, b) => a - b)
  if (successfulPings.length === 0) return null
  const median = successfulPings[Math.floor(successfulPings.length / 2)]
  if (median === 0) return 0
  const spikes = successfulPings.filter(ms => ms > median * 2).length
  return Math.round((spikes / successfulPings.length) * 100)
}
...
export const getStabilityScore = (r, windowMs = DEFAULT_PING_WINDOW_MS) => {
  const p95 = getP95(r, windowMs)
  const jitter = getJitter(r, windowMs)
  const spikeRate = getSpikeRate(r, windowMs)
  const uptime = getUptime(r)
  
  if (p95 === null || jitter === null || spikeRate === null) {
    return null
  }
  
  const p95Score = Math.max(0, 100 - (p95 / 50))
  const jitterScore = Math.max(0, 100 - (jitter / 20))
  const spikeScore = Math.max(0, 100 - spikeRate)
  const uptimeScore = uptime
  
  const score = 
    (p95Score * STABILITY_WEIGHTS.p95) +
    (jitterScore * STABILITY_WEIGHTS.jitter) +
    (spikeScore * STABILITY_WEIGHTS.spikeRate) +
    (uptimeScore * STABILITY_WEIGHTS.uptime)
